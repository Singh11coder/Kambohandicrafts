...
router.get('/', async (req, res) => {
  const { q, category, magnetic, round, folding, minSize, maxSize, minPrice, maxPrice } = req.query as Record<string, string>;

  const where: any = {};
  if (q) where.OR = [{ name: { contains: q, mode: 'insensitive' } }, { description: { contains: q, mode: 'insensitive' } }];
  if (category) where.category = { name: category };
  if (magnetic !== undefined) where.isMagnetic = magnetic === 'true';
  if (round !== undefined) where.isRound = round === 'true';
  if (folding !== undefined) where.isFolding = folding === 'true';
  if (minSize) where.sizeInches = { ...(where.sizeInches || {}), gte: Number(minSize) };
  if (maxSize) where.sizeInches = { ...(where.sizeInches || {}), lte: Number(maxSize) };
